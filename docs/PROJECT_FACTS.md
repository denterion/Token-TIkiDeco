...
| `npm run deps:audit` runs the npm advisory scan only; it is not an independent smart-contract audit. The legacy `npm run audit` aggregate also generates the V2 candidate review package. | `package.json` (`deps:audit`, `audit:deps`, `audit`, `audit:package`). | 2026-07-10 | verified |
...
| `docs/reports/REPORT_2026_07_11_V02_FINAL_EVIDENCE.md` records the v0.2 evidence refresh and review-bundle hashes for source commit `00e0ccbbf26397aadd5416816bba44e7cc2e373b`. | Repository file `docs/reports/REPORT_2026_07_11_V02_FINAL_EVIDENCE.md`; SHA-256 in `docs/reports/REPORT_2026_07_11_V02_FINAL_EVIDENCE_HASH.md`; local review bundle generated under `release-artifacts/v0.2.0-utility-pilot/00e0ccbbf26397aadd5416816bba44e7cc2e373b`. | 2026-07-11 | verified |
| `docs/reports/REPORT_2026_07_09_PILOT_PROOF_DRY_RUN.md` records an aggregate-only pilot proof dry run using fake/test addresses. | Repository file `docs/reports/REPORT_2026_07_09_PILOT_PROOF_DRY_RUN.md`; SHA-256 in `docs/reports/REPORT_2026_07_09_PILOT_PROOF_DRY_RUN_HASH.md`; dry-run artifacts under `operations/utility-pilot/dry-runs/`. | 2026-07-09 | verified |
...
| The current public 3D site uses English UI copy only; the earlier EN/ES/RU localization remains part of historical release records, not the current interface. | `site-v2/src/data/i18n.ts`; `site-v2/src/main.tsx`; `site/index.html`; historical report `docs/reports/REPORT_2026_06_24_PUBLIC_RELEASE_AND_LOCALIZATION.md`. | 2026-07-11 | verified |
...
| A v0.2 utility-pilot review bundle was reproduced for source commit `00e0ccbbf26397aadd5416816bba44e7cc2e373b`; later metadata commits are not part of that immutable source archive. | `config/release-evidence.json`; `docs/releases/v0.2.0-utility-pilot-rc.1.md`; `docs/reports/REPORT_2026_07_11_V02_FINAL_EVIDENCE.md`. | 2026-07-11 | verified |
| `v0.2.1-public-review` is a draft pre-release evidence layer for exact main snapshot `3d5c207b133e4b86459bdc173b78422315a0c744`; its report is preserved at `docs/reports/immutable/v0.2.1-public-review/MONTHLY_REPORT_2026_07.md`; it is not a published tag and does not rewrite either earlier pre-release. | `config/public-versions.json`; `config/release-evidence.json`; `config/releases/v0.2.1-public-review.json`; `docs/releases/v0.2.1-public-review.md`; `git rev-parse origin/main`. | 2026-07-12 | verified |
| The July monthly transparency report is generated from repository and GitHub evidence; zero recorded community findings is explicitly not treated as proof of safety. | `docs/reports/MONTHLY_REPORT_2026_07.md`; `scripts/generate-monthly-transparency-report.cjs`; `scripts/check-monthly-transparency-report.cjs`. | 2026-07-12 | verified |
| The Safe resilience drill is test-only and did not change the Safe threshold or broadcast a transaction. | `docs/governance/SAFE_RESILIENCE_DRILL_2026.md`; `operations/governance/safe-drill-result.json`; `config/governance/safe-resilience.json`. | 2026-07-12 | verified |
| The operator sandbox uses fake local data and is not a real hotel service or active guest benefit. | `docs/hospitality-operator/OPERATOR_SANDBOX_DEMO.md`; `operations/hospitality-operator/operator-sandbox-report.json`; `scripts/run-operator-sandbox.cjs`. | 2026-07-12 | verified |
...
| Public first-reader navigation is defined as four entry points: Overview, Status, Pilot, and Audit, with Feedback as the action path. | `docs/PUBLIC_ENTRYPOINTS.md`; `docs/START_HERE.md`; `site-v2/src/main.tsx`; `scripts/build-site-pages.cjs`. | 2026-07-08 | verified |
| A public Trust Center links current main, immutable pre-release tags, evidence metadata, canonical V1 facts, candidate V2 status, operator/entity status, limitations, security disclosure, and feedback to their sources. | `site/trust/index.html`; `docs/TRUST_CENTER_SOURCE_MAP.md`; `scripts/check-trust-center.cjs`. | 2026-07-10 | verified |
| The public preview UI records only short-lived in-memory aggregate counters and does not persist or transmit raw wallet addresses for analytics. | `site-v2/src/lib/previewMetrics.ts`; `scripts/check-preview-privacy.cjs`; `docs/utility-pilot/PUBLIC_PREVIEW_PRIVACY_THREAT_MODEL.md`. | 2026-07-10 | verified |
| A zero-sample public preview proof baseline exists and explicitly reports that no public responses have been collected. | `docs/reports/REPORT_2026_07_10_PUBLIC_PREVIEW_PROOF.md`; `docs/reports/REPORT_2026_07_10_PUBLIC_PREVIEW_PROOF_HASH.md`; `config/utility-pilot/public-preview-metrics-baseline.json`. | 2026-07-10 | verified |
| The repository is publicly owned by the GitHub user account `denterion`; no legal entity or hospitality operator is publicly verified by repository evidence. | GitHub repository metadata; `docs/OPERATOR_AND_ENTITY_STATUS.md`. | 2026-07-10 | verified |
| GitHub Issues are enabled, Discussions are disabled, and GitHub private vulnerability reporting is enabled. | GitHub repository settings checked through the GitHub API; `docs/PUBLIC_PARTICIPATION.md`; `SECURITY.md`. | 2026-07-10 | verified |
...
| A limited Sepolia preview evidence checklist and live-gate owner matrix exist; they keep every gate tied to an owner, evidence file, blocking reason, reviewer, approval format, and checking command while the campaign remains blocked. | `docs/utility-pilot/LIMITED_SEPOLIA_PREVIEW_EVIDENCE.md`; `docs/utility-pilot/LIVE_GATE_OWNER_MATRIX.md`; `scripts/check-pilot-evidence.cjs`; `npm run pilot:evidence`. | 2026-07-08 | planned |
| A Sepolia allocation dry run exists using fake/test addresses. It generates a Safe Transaction Builder JSON draft and an aggregate-only report draft; no transaction is broadcast. | `operations/utility-pilot/dry-runs/limited-preview-allocation-input.json`; `operations/utility-pilot/dry-runs/limited-preview-safe-transaction-builder-draft.json`; `operations/utility-pilot/dry-runs/limited-preview-allocation-report-draft.json`; `scripts/prepare-testnet-allocation.cjs`. | 2026-07-08 | planned |
...
| An Official Public Preview packet exists for community, auditor, operator, governance, release, and legal/privacy reviewers. | `docs/OFFICIAL_PUBLIC_PREVIEW.md`; `README.md`; `docs/START_HERE.md`. | 2026-07-04 | verified |
...
| The campaign lifecycle supports draft, evidence review, approved testnet preview, paused, closed, and archived stages, but the current stage remains `draft`. | `config/utility-pilot/tide-community-preview-001.json`; `config/utility-pilot/campaign-lifecycle.schema.json`; `npm run preview:check`. | 2026-07-10 | planned |
| Early RSVP eligibility is the first documented future non-financial example, but operator, inventory, support, availability, and legal approval remain absent or blocked. | `docs/utility-pilot/FUTURE_BENEFIT_SPEC.md`. | 2026-07-10 | planned |
| A pilot proof pack exists as a short review path for the planned Sepolia utility pilot. It does not publish a live campaign. | `docs/PILOT_PROOF_PACK.md`; `docs/reports/REPORT_2026_07_09_PILOT_PROOF_DRY_RUN.md`; `site/proof/index.html`. | 2026-07-09 | planned |
...
| A TikiDeco legal entity or commercial hospitality operator is established. | No formation record, operator agreement, or counsel confirmation is published; see `docs/OPERATOR_AND_ENTITY_STATUS.md`. | 2026-07-10 | unknown |
