...
- `accepted-for-review`: current behavior is intentionally kept for independent technical review.
...
- `ask-reviewer`: keep the issue in review scope and ask the reviewer to assess tradeoffs.
- `blocked-before-deployment`: do not use a public V2 candidate deployment until resolved.
- `resolved`: the recorded mitigation and regression evidence are complete for this review candidate.
...
| KI-03: V2 vault has no on-chain pause role | `ask-reviewer` | Current freeze uses token-pause-only as the incident circuit breaker. This keeps one global transfer stop, but it can delay beneficiary releases during token pause. | Accepted for review package only. Reviewer question: should V2 add a vault-local pause role before any deployment decision? | Yes, explicit reviewer question. | Existing: `test/TikiDecoVestingVaultV2.js` release while token paused. Missing if changed: vault-local pause tests for release, revoke, and treasury transfer. | Issue #62; `docs/V2_AUDIT_TARGET_FREEZE.md#fd-02-vault-pause-model`. |
| KI-04: Report supersede graph policy | `accepted-for-review` | On-chain validation requires existing report ID and emits a supersede event. Semantic correction reason can remain off-chain for candidate scope. | Accepted risk: reporter can publish unrelated corrections if governance/report policy is weak. Planned control: repository report policy must require correction context. | Yes, ask whether same-category or correction-reason hash should become on-chain. | Existing: `test/TikiDecoTokenV2.js` report and supersede event tests. Missing: off-chain report-policy lint for correction reason. | Issue #62; future reporting-policy PR if reviewer recommends on-chain change. |
| KI-05: Public site RPC availability | `accepted-for-review` | Site is read-only and must show unavailable/cached/stale states instead of invented values. This is mostly frontend trust risk, not V2 contract risk. | Accepted risk: public RPC outage can reduce dashboard usefulness. Planned control: keep explorer/source links prominent and avoid zero fallback data. | Optional, not primary Solidity scope. | Existing: `site-v2/tests/site-regression.spec.ts` RPC unavailable and mocked balance tests; `npm run site`. | Issue #66 / site feedback issues. |
| KI-06: Slither baseline model | `accepted-for-review` | Known V2 Slither findings are versioned and explained. New untriaged V2 findings should fail the gate. | Accepted risk: accepted findings remain visible, not hidden. Planned control: `npm run slither` and `npm run slither:baseline` before handoff. | Yes, review accepted classifications. | Existing: `security/slither-baseline-v2.json`; `scripts/check-slither-baseline.cjs`; `npm run slither`. | Issue #62. |
| KI-07: Bytecode size after coverage artifacts | `accepted-for-review` | Coverage can influence artifacts; release evidence should run clean compile before bytecode. | Accepted operational risk with runbook control. Planned control: fresh-checkout proof runs compile before bytecode and package generation. | Optional, release-process review. | Existing: `npm run lint`; `scripts/check-bytecode-size.cjs`. Missing: independent reproduction of the documented sequence. | `docs/FRESH_CHECKOUT_RELEASE_PROOF.md`. |
...
| KI-09: Release package local Foundry path and generated site output | `resolved` | Package tooling handles the repository-pinned Foundry path and dynamically restores generated files under the verified `site/` directory. | Exact-main release proof now leaves the tree clean; any regression fails the postflight clean-tree assertion. | Reviewer may reproduce package checksums; no open deployment remediation remains for this item. | Existing: `scripts/generate-audit-release-package.cjs`; `scripts/prove-release-reproducibility.cjs`; exact-main release proof at `9c74d225ad3b5cdb8e2ae59ea9a53e36f5d5f075`. | PR #113. |

## Deployment Impact Register

| ID | Deployment impact |
| --- | --- |
| KI-01 | Blocks deployment metadata approval until exact neutral strings are reviewed. |
| KI-02 | Blocks any V2 deployment until real role addresses, Safe threshold and post-deployment assertions are reviewed. |
| KI-03 | Does not block independent review; remains an explicit reviewer decision before deployment. |
| KI-04 | Does not block review; a reviewer recommendation may require a scoped contract or reporting-policy change before deployment. |
| KI-05 | Outside contract deployment readiness; remains a public availability and trust consideration. |
| KI-06 | Does not block review while accepted findings remain visible and new untriaged findings equal zero. |
| KI-07 | Does not block review; release sequencing must remain part of deployment evidence. |
| KI-08 | Blocks deployment promotion until dependency warnings are removed or explicitly accepted. |
| KI-09 | No remaining deployment impact for the frozen candidate; release cleanliness is regression-checked. |
