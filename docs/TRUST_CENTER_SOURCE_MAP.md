...
| Aggregate community findings | [`config/community-review/findings.json`](../config/community-review/findings.json) and [`community-review/FINDING_TRIAGE_PLAYBOOK.md`](community-review/FINDING_TRIAGE_PLAYBOOK.md) | `npm run findings:check` | Public-safe aggregate counts for the exact candidate; sensitive unresolved details are excluded. | Show `Unavailable` when registry validation fails; never infer zero findings from missing data. |
...
| Latest monthly transparency report | [`reports/MONTHLY_REPORT_LATEST.md`](reports/MONTHLY_REPORT_LATEST.md) and its machine-readable JSON | `npm run transparency:monthly:check` | Dated source-linked monthly repository snapshot. | Show unavailable when missing; never present an older month as current. |
